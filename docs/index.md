# go-ruby-puma documentation

**Pure-Go (CGO=0) MRI-faithful reimplementation of Ruby's puma — the threaded Rack web server — on the Go net/http stack.**

`go-ruby-puma/puma` is a faithful, pure-Go (zero cgo) reimplementation of Ruby's `puma`,
matching reference Ruby (MRI) behaviour. The module path is
`github.com/go-ruby-puma/puma`.
...
!!! success "Status: pure-Go, CGO=0, differential-tested"
    A faithful pure-Go port of Ruby's `puma`, validated against reference Ruby, at 100%
...
```sh
go get github.com/go-ruby-puma/puma
```
...
| Repo | What it is |
| --- | --- |
| [`puma`](https://github.com/go-ruby-puma/puma) | the library — Ruby's `puma` in pure Go |
| [`docs`](https://github.com/go-ruby-puma/docs) | this documentation site (MkDocs Material, versioned with mike) |
| [`go-ruby-puma.github.io`](https://github.com/go-ruby-puma/go-ruby-puma.github.io) | the organization landing page (Hugo) |
| [`brand`](https://github.com/go-ruby-puma/brand) | logo and brand assets |
...
Source lives at [github.com/go-ruby-puma/puma](https://github.com/go-ruby-puma/puma).
