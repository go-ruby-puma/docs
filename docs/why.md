...
`go-ruby-puma/puma` reimplements Ruby's `puma` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-puma/puma` directly, with no Ruby runtime;
